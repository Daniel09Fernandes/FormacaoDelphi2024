...
begin
  inherited;
  var Pedidos := FController.BuscarPedidos;

  MemPedidos.Open;
  for var I := 0 to Pedidos.Count -1 do
     MemPedidos.AppendRecord([
           Pedidos.Items[i].IdPedido,
           Pedidos.Items[i].NomeCliente,
           Pedidos.Items[i].TotalPedido,
           Pedidos.Items[i].IdCliente
     ]);

   var ItensPedidos := FController.BuscarItensPedidos;

   MemItensPedidos.Open;
   for var I := 0 to ItensPedidos.Count do
     MemItensPedidos.AppendRecord([
                   ItensPedidos.Items[i].IdItem,
                   ItensPedidos.Items[i].NomeProduto,
                   ItensPedidos.Items[i].IdProduto,
                   ItensPedidos.Items[i].IdPedido,
                   ItensPedidos.Items[i].Preco
                  ]);
end;
